...
def c4_process(args):
    if args.spark_archives:
        ...
        spark = SparkSession.builder.master(args.spark_master)\
    ...
    wet_file_paths = spark.sparkContext.parallelize(
        glob.glob("/juicefs/gpt/data.commoncrawl.org/crawl-data/CC-MAIN-2022-40/segments/*/wet/*.gz"))

    def chinese_filter(text):
        try:
            if langdetect.detect(text) not in ["zh-cn", "zh-tw"]:
                return False
        except:
            return False
    ...
    page_content = wet_file_paths\
        ...
        .filter(lambda doc: chinese_filter(doc.text))\
        .map(lambda doc: dict(
            url=doc.url,
            text=doc.text,
